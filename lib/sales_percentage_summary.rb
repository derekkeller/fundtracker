class SalesPercentageSummary
  ...
  PeriodSet = Struct.new(:gross_margin, :operating_margin, :cash_burn, :year, :label, :bookings, :revenue, :revenue_line_items, :cogs, :cogs_line_items, :gross_profit, :operating_expense, :sga, :r_and_d, :total_operating_expenses, :expense_line_items, :operating_profit, :interest_income, :other_income, :tax_expense, :misc_line_items, :total_miscellaneous, :net_income, :net_margin, :_total_revenue)  
  ...
  def initialize(periods, company)
    ...
    @periods.each do |p|
      ...
      begin
        ...
        @period_set << PeriodSet.new(dbr(@gross_margin), dbr(@operating_margin), dbr(@cash_burn), p.first.year, p, dbr(@bookings), dbr(@revenue), @revenue_line_items, dbr(@cogs), @cogs_line_items, dbr(@gross_profit), dbr(@operating_expenses), dbr(@sga), dbr(@r_and_d), dbr(@total_operating_expenses), @expense_line_items, dbr(@operating_profit), dbr(@interest_income), dbr(@other_income), dbr(@tax_expense), @misc_line_items, dbr(@total_miscellaneous), dbr(@net_income), dbr(@net_margin), @total_revenue)
    end
  end
  
  def divide_by_total_revenue(amount)
    if amount.class == Array
      #do nothing
    else
      amount / @total_revenue.to_f
    end
  
  end
  
  alias dbr divide_by_total_revenue
